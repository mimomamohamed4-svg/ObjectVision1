...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Mono:ital,wght@0,300;0,400;0,500&family=Inter:wght@300;400;500;600;700&display=swap');
...
.seccion { padding: 30px 80px; }
@media (max-width: 768px) {
    .seccion { padding: 20px 16px !important; }
    .hero { padding: 30px 16px 24px 16px !important; }
    .hero-title { font-size: 2.8rem !important; }
    .hero-sub { font-size: 0.9rem !important; }
    .stats-bar { gap: 20px !important; flex-wrap: wrap; }
    .stat-number { font-size: 1.8rem !important; }
    .bottom-bar { padding: 20px 16px !important; flex-direction: column; gap: 12px; text-align: center; }
    .stTabs [data-baseweb="tab-list"] { padding-left: 16px !important; gap: 8px !important; }
    .stTabs [data-baseweb="tab"] { font-size: 0.7rem !important; height: 44px !important; }
}
...
.result-item { padding: 18px 0; border-bottom: 1px solid #1a2744; }
.result-item:last-child { border-bottom: none; }
.result-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; }
.result-name { font-size: 1.05rem; font-weight: 600; color: #e8eaf0; font-family: 'Inter', sans-serif; }
...
.rank-badge { font-family: 'DM Mono', monospace; font-size: 0.65rem; padding: 3px 8px; border-radius: 6px; margin-right: 10px; font-weight: 500; }
.history-card { background: #0d1422; border: 1px solid #1a2744; border-radius: 12px; padding: 14px; display: flex; align-items: center; gap: 14px; margin-bottom: 12px; }
.history-thumb { width: 55px; height: 55px; object-fit: cover; border-radius: 8px; flex-shrink: 0; }
.history-name { font-weight: 600; color: #e8eaf0; font-size: 0.9rem; font-family: 'Inter', sans-serif; }
.history-meta { font-size: 0.72rem; color: #4a6080; font-family: 'DM Mono', monospace; margin-top: 4px; }
.empty-state { display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 200px; gap: 12px; background: #090f1a; border-radius: 16px; border: 1px dashed #1a2744; padding: 20px; }
.empty-icon { font-size: 2rem; opacity: 0.15; color: #0066ff; }
.empty-text { font-size: 0.8rem; color: #4a6080; font-family: 'DM Mono', monospace; text-align: center; }

.hero { background: linear-gradient(135deg, #080c14 0%, #0d1829 100%); padding: 60px 80px 44px 80px; border-bottom: 1px solid #1a2744; }
.hero-title { font-family: 'Bebas Neue', sans-serif; font-size: clamp(3rem, 7vw, 6rem); font-weight: 400; line-height: 1; letter-spacing: 4px; color: #fff; max-width: 900px; margin-bottom: 16px; }
...
.hero-sub { font-size: 1rem; color: #6b7c96; max-width: 520px; line-height: 1.7; margin-bottom: 36px; font-family: 'Inter', sans-serif; font-weight: 300; }
.stats-bar { display: flex; gap: 40px; padding-top: 28px; border-top: 1px solid #1a2744; flex-wrap: wrap; }
.stat-number { font-family: 'Bebas Neue', sans-serif; font-size: 2rem; font-weight: 400; color: #fff; letter-spacing: 2px; }
.stat-label { font-size: 0.66rem; color: #4a6080; letter-spacing: 2px; text-transform: uppercase; margin-top: 2px; font-family: 'DM Mono', monospace; }

.bottom-bar { padding: 24px 80px; border-top: 1px solid #1a2744; display: flex; justify-content: space-between; align-items: center; background: #05080f; flex-wrap: wrap; gap: 10px; }
.bottom-left { font-size: 0.75rem; color: #4a6080; font-family: 'DM Mono', monospace; }
.bottom-tag { font-size: 0.7rem; color: #4a6080; font-family: 'DM Mono', monospace; letter-spacing: 1px; margin-left: 20px; }
...
.stTabs [data-baseweb="tab-list"] { gap: 20px; padding-left: 80px; border-bottom: 1px solid #1a2744; background: #060a10; overflow-x: auto; }
.stTabs [data-baseweb="tab"] { height: 50px; background-color: transparent !important; color: #4a6080 !important; font-family: 'DM Mono', monospace; font-size: 0.78rem; font-weight: 500; white-space: nowrap; letter-spacing: 1px; }
.stTabs [data-baseweb="tab"][aria-selected="true"] { color: #fff !important; border-bottom-color: #0066ff !important; }
.stButton > button { background: rgba(0,102,255,0.08) !important; color: #0066ff !important; border: 1px solid rgba(0,102,255,0.3) !important; border-radius: 8px !important; font-family: 'DM Mono', monospace !important; font-size: 0.72rem !important; font-weight: 500 !important; text-transform: uppercase !important; letter-spacing: 1px !important; }
...
if not st.session_state.autenticado:
    ...
    with col2:
        ...
        <div style="text-align:center; margin-bottom:24px;">
            ...
            <div style="font-size:0.68rem; color:#2a3a54; letter-spacing:3px; text-transform:uppercase; margin-top:8px; font-family:'DM Mono',monospace;">
...
with nav_col2:
    st.markdown("""
    <div style="display:flex;gap:6px;padding-top:8px;flex-wrap:wrap;">
        ...
        <span style="font-size:0.6rem;letter-spacing:1px;text-transform:uppercase;color:#4a6080;background:rgba(26,39,68,0.5);border:1px solid #1a2744;padding:3px 8px;border-radius:4px;font-family:'DM Mono',monospace;">ImageNet</span>
    </div>
    """, unsafe_allow_html=True)
with nav_col3:
    st.markdown(f"""
    <div style="display:flex;align-items:center;justify-content:flex-end;padding-top:8px;">
        <span style="font-family:'DM Mono',monospace;font-size:0.68rem;color:#00d4aa;">
            <i class="fa fa-circle" style="font-size:0.5rem;margin-right:4px;"></i> {st.session_state.rol_usuario}
        </span>
    </div>
    """, unsafe_allow_html=True)
    ...
    with sub_c3:
        if st.button("FR", key="btn_fr"):
            st.session_state.idioma = "fr"; st.rerun()
    with sub_c4:
...
TEXTOS = {
    "es": {
        ...
        "tab_analizar": "Analizar", "tab_camara": "Cámara",
        "tab_comparar": "Comparar", "tab_historial": "Historial",
        ...
        "modelo_a": "— MobileNetV2", "modelo_b": "— ResNet50",
    ...
    "en": {
        ...
        "tab_analizar": "Analyze", "tab_camara": "Camera",
        "tab_comparar": "Compare", "tab_historial": "History",
    ...
    "fr": {
        ...
        "tab_analizar": "Analyser", "tab_camara": "Caméra",
        "tab_comparar": "Comparer", "tab_historial": "Historique",
...
def mostrar_resultados(top3, t, idm, umbral):
        ...
        <div class="result-item">
            <div class="result-header">
                <div style="display:flex;align-items:center;min-width:0;">
                    <span class="rank-badge" style="background:{badge_colors[i]}22;color:{badge_colors[i]};border:1px solid {badge_colors[i]}44;flex-shrink:0;">#{str(i+1).zfill(2)}</span>
                    <span class="result-name" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{nombre}</span>
                </div>
                <span class="result-pct" style="color:{badge_colors[i]};flex-shrink:0;margin-left:8px;">{pct:.1f}%</span>
            </div>
            <div class="bar-track">
                <div class="bar-fill" style="width:{pct}%;background:{color_bar}"></div>
            </div>
            <div style="margin-top:6px;font-size:0.68rem;color:#4a6080;font-family:'DM Mono',monospace;letter-spacing:1px">{nivel}</div>
        </div>
        """, unsafe_allow_html=True)

    if visibles == 0:
        st.markdown(f'<div class="empty-state"><div class="empty-icon"><i class="fa fa-triangle-exclamation"></i></div><div class="empty-text">Ningún objeto supera el {umbral}%.</div></div>', unsafe_allow_html=True)
        return
    ...
    with col_b1:
        if st.button(f"▶ {t['boton_voz']}", key=f"voz_{nombre_top}_{idm}"):
...
<div class="hero">
    ...
    <div class="stats-bar">
        ...
        <div><div class="stat-number">Cloud</div><div class="stat-label">Remoto</div></div>
        <div><div class="stat-number">24/7</div><div class="stat-label">Disponible</div></div>
    </div>
</div>
""", unsafe_allow_html=True)
...
with tabs_render[0]:
    ...
    with col2:
        ...
        if archivo:
            ...
            st.markdown(f"<span style='font-family:DM Mono;font-size:0.68rem;color:#00d4aa;letter-spacing:1px'>&#9889; {ms:.0f}ms</span>", unsafe_allow_html=True)
            ...
            st.markdown(f'<div class="empty-state"><div class="empty-icon"><i class="fa fa-image"></i></div><div class="empty-text">{t["esperando"]}</div></div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
with tabs_render[1]:
    ...
    with col2:
        ...
        if foto:
            ...
            st.markdown(f'<div class="empty-state"><div class="empty-icon"><i class="fa fa-camera"></i></div><div class="empty-text">{t["camara_info"]}</div></div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
with tabs_render[2]:
    ...
    if archivo_comp:
        ...
        st.markdown(f'<div class="empty-state"><div class="empty-icon"><i class="fa fa-code-compare"></i></div><div class="empty-text">{t["comparar_info"]}</div></div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
with tabs_render[3]:
    ...
    if st.session_state.historial:
        ...
        st.markdown(f'<div class="empty-state"><div class="empty-icon"><i class="fa fa-clock-rotate-left"></i></div><div class="empty-text">{t["historial_vacio"]}</div></div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

if es_admin:
    with tabs_render[4]:
        st.markdown("<div class='seccion'>", unsafe_allow_html=True)
        st.markdown('<div class="zone-label">— Panel Admin</div>', unsafe_allow_html=True)
        st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
        filas = ""
        for usr, datos in st.session_state.usuarios_db.items():
            ...
            filas += f"""<tr>
                <td style="padding:12px;border-bottom:1px solid #1a2744;color:#e8eaf0;font-family:'DM Mono',monospace;font-weight:500;font-size:0.8rem">{usr}</td>
                <td style="padding:12px;border-bottom:1px solid #1a2744;color:#4a6080;font-family:'DM Mono',monospace;font-size:0.8rem">{clave_oculta}</td>
                <td style="padding:12px;border-bottom:1px solid #1a2744"><span style="font-family:'DM Mono',monospace;font-size:0.68rem;padding:3px 8px;border-radius:4px;{badge}">{datos['rol']}</span></td>
            </tr>"""
        st.components.v1.html(f"""
        ...
        <table style="width:100%;border-collapse:collapse;background:#0d1422;border-radius:8px;border:1px solid #1a2744;">
            <thead><tr style="background:#111a2e;color:#0066ff;font-family:'DM Mono',monospace;font-size:0.72rem;letter-spacing:1px;text-align:left">
                <th style="padding:10px 12px"><i class="fa fa-user"></i> USUARIO</th>
                <th style="padding:10px 12px"><i class="fa fa-lock"></i> CONTRASEÑA</th>
                <th style="padding:10px 12px"><i class="fa fa-shield"></i> ROL</th>
            </tr></thead>
            <tbody>{filas}</tbody>
        </table>
